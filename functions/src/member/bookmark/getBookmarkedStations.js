...
exports.getBookmarkedStations = functions.https.onRequest(async (req, res) => {
  ...
  await authenticateToken(req, res, async () => {
    try {
      ...
      // bookmark_stations 컬렉션에서 사용자의 북마크 데이터 조회
      const bookmarksSnapshot = await db.collection('bookmark_stations')
        .where('userId', '==', userId)
        .get();

      // rental_stations와 JOIN하여 스테이션 정보 가져오기
      const bookmarks = await Promise.all(
        bookmarksSnapshot.docs.map(async doc => {
          const stationDoc = await db.collection('rental_stations')

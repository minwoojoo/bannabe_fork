...
exports.getNearbyStations = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const lat_range = radiusInKm / 111.32; // 1도 = 약 111.32km
    const lng_range = radiusInKm / (111.32 * Math.cos(latitude * (Math.PI / 180)));

    // 주변 스테이션 조회
    const stationsSnapshot = await db.collection('stations')
      .where('location.latitude', '>=', latitude - lat_range)
      .where('location.latitude', '<=', latitude + lat_range)
      .get();
    ...
    const stations = stationsSnapshot.docs
      ...
      .filter(station => {
        const lng_diff = Math.abs(station.location.longitude - longitude);
        return lng_diff <= lng_range;
      })
      .map(station => ({
        name: station.name,
